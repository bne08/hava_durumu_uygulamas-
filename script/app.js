...
const guncelleUI = (veri) => {
  ...
  const { sehirDetay, havaDurumu } = veri; //yukardakını tek satırda yazdık
  ...
  const iconSrc = `image/icons/${havaDurumu.WeatherIcon}.png`;
  icon.setAttribute("src", iconSrc);

  let zamanSrc = null;
  if (havaDurumu.IsDayTime) {
    zamanSrc = "image/day.jpg";
  } else {
    zamanSrc = "image/night.jpg";
  }

  //let zamanSrc=havaDurumu.IsDayTime?'image/day.jpg':'image/night.jpg';
